...
def add_compute(config, name, mgmt_ip, tenant_ip, public_ip)
...
def add_controller(config, name, mgmt_ip)
  config.vm.box_check_update = false
  config.vbguest.auto_update = true
  config.ssh.forward_x11 = true
  config.vm.define name do |node|
...
Vagrant.configure(2) do |config|
  ...
  add_compute(config, "compute0", "192.168.255.100/24", "10.10.10.100/24", "10.0.0.100/24")
  add_compute(config, "compute1", "192.168.255.101/24", "10.10.10.101/24", "10.0.0.101/24")
  add_compute(config, "compute2", "192.168.255.102/24", "10.10.10.102/24", "10.0.0.102/24")
end
